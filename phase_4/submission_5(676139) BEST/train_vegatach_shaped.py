...
def collect_episode(env_cls, args, seed, q_net, device, epsilon):
    ...
    while not done:
        ...
        # ---------- Blind-state shaping ----------
        # Blind = no object/wall sensor bits active, and not stuck.
        # Use next observation so shaping depends on the result of the action.
        shape = 0.0
        is_blind = (raw_obs[:16].sum() == 0) and (raw_obs[17] < 0.5)

        # Apply shaping only before attachment, so push behaviour stays untouched.
        if (not ep.got_attachment) and is_blind:
            if a == 2:  # FW
                shape += 0.2
                blind_turn_streak = 0
            else:  # turn actions
                blind_turn_streak += 1
                if blind_turn_streak >= 9:
                    shape -= 0.2
                elif blind_turn_streak >= 5:
                    shape -= 0.1
        else:
            blind_turn_streak = 0
        # ----------------------------------------
