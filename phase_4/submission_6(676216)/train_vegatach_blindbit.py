...
class EpisodeReplay:
    ...
    def sample_batch(self, batch_size):
        ...
        probs /= probs.sum()
        ep_indices = np.random.choice(len(self.episodes), size=batch_size, p=probs, replace=True)
...
def make_obs(raw_obs, prev_action, blind_steps, max_steps):
    one_hot = np.zeros(PREV_ACT_DIM, dtype=np.float32)
    if prev_action >= 0:
        one_hot[prev_action] = 1.0
    blind_norm = np.array([min(blind_steps / max_steps, 1.0)], dtype=np.float32)
    return np.concatenate([raw_obs.astype(np.float32), one_hot, blind_norm])
...
EXPLORE_BLIND = np.array([0.05, 0.05, 0.80, 0.05, 0.05])
...
def collect_episode(env_cls, args, seed, q_net, device, epsilon):
    ...
    prev_action = -1
    blind_steps = 0
    is_blind = raw_obs[:17].sum() == 0
    if is_blind:
        blind_steps = 1

    aug = make_obs(raw_obs, prev_action, blind_steps, args.max_steps)
    ep.obs.append(aug)
    ...
    while not done:
        if np.random.rand() < epsilon:
            if raw_obs[17] > 0.5:
                ...
                a = int(np.random.choice(N_ACTIONS, p=EXPLORE_BLIND))
            else:
...
def _run_eval(q_net, env_cls, args, device, wall_obstacles, difficulty, seeds=range(10)):
    ...
    for seed in seeds:
        ...
        while not done:
            aug = make_obs(raw_obs, prev_action, blind_steps, 2000)
            with torch.no_grad():
                obs_t = torch.tensor(aug, dtype=torch.float32, device=device).reshape(1, 1, -1)
                q_vals, h = q_net(obs_t, h)
...
def main():
    ...
    train_log = "train_log_blind.csv"
    eval_log = "eval_log_blind.csv"
    ...
    for ep in pbar:
        ...
        if (ep + 1) % args.eval_every == 0:
            ...
            if overall_mean > best_eval:
                ...
                torch.save(q_net.state_dict(), os.path.join(save_dir, f"best{ep+1}.pth"))
    ...
    torch.save(q_net.state_dict(), os.path.join(save_dir, "final.pth"))
    print("Done.")
