...
def _load_once():
    ...
    find_q = FindDDQN()
    find_q.load_state_dict(torch.load(os.path.join(submission_dir, "best_find_ep1300.pth"), map_location="cpu"))
    find_q.eval()

    push_ckpt = torch.load(os.path.join(submission_dir, "push_ddqn.pth"), map_location="cpu")
    push_q = PushDDQN()
...
def _reset_episode(obs):
    torch = _STATE['torch']
    prev_act = np.zeros(N_ACTIONS, dtype=np.float32)
    aug = np.concatenate([obs, [0.0], prev_act])
    _STATE['frames'].clear()
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    torch = _STATE['torch']
    find_q = _STATE['find_q']
    push_q = _STATE['push_q']
    ...
    if _STATE['last_rng_id'] != rng_id:
        ...
        prev_act = np.zeros(N_ACTIONS, dtype=np.float32)
        if _STATE['prev_action'] is not None:
            prev_act[_STATE['prev_action']] = 1.0
        aug = np.concatenate([obs, [_STATE['pred_attached']], prev_act])
    ...
    with torch.no_grad():
        ...
        if _STATE['pred_attached'] < 0.5:
            qs = find_q(x).squeeze(0).numpy()
        else:
            qs = push_q(x).squeeze(0).numpy()
