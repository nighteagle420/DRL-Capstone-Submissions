...
CURRICULUM = [
    {"difficulty": 0, "wall_obstacles": False, "label": "d0_nowall"},
    {"difficulty": 0, "wall_obstacles": True,  "label": "d0_wall"},
    {"difficulty": 2, "wall_obstacles": True,  "label": "d2_wall"},
    {"difficulty": 3, "wall_obstacles": True,  "label": "d3_wall"},
]
...
def collect_episode(env_cls, args, seed, q_net, device, epsilon, difficulty, wall_obstacles):
    env = env_cls(
...
def train_step(q_net, tgt_net, replay, optimizer, device, args):
    ...
    obs_learn = obs_t[:, burn:SL, :]
    acts_learn = acts_t[:, burn:]
    rews_learn = rews_t[:, burn:]
    dones_learn = dones_t[:, burn:]

    obs_next_learn = obs_t[:, burn + 1:SL + 1, :]
...
def main():
    ...
    ap.add_argument("--max_replay_episodes", type=int, default=4000)
    ...
    ap.add_argument(
        ...
        type=int,
        nargs=4,
        # default=[800, 800, 1000, 1400],
        default=[600,1200,1400,1800],
        help="episodes per curriculum stage [d0_nowall, d0_wall, d2_wall, d3_wall]",
    )
    ...
    stage_boundaries = []
    cumulative = 0
    for se in args.stage_episodes:
        cumulative += se
        stage_boundaries.append(cumulative)
    args.episodes = cumulative
    ...
    print(f"Curriculum stages: {args.stage_episodes}, boundaries: {stage_boundaries}")
    for i, stage in enumerate(CURRICULUM):
        print(f"  Stage {i+1}: {stage['label']} ({args.stage_episodes[i]} eps)")
    ...
    for ep in pbar:
        while current_stage < len(stage_boundaries) - 1 and ep >= stage_boundaries[current_stage]:
            current_stage += 1
            torch.save(q_net.state_dict(), os.path.join(save_dir, f"end_stage{current_stage}.pth"))
            tqdm.write(f"=== Advancing to stage {current_stage+1}: {CURRICULUM[current_stage]['label']} ===")

        stage = CURRICULUM[current_stage]
        difficulty = stage["difficulty"]
        wall_obstacles = stage["wall_obstacles"]

        stage_start = stage_boundaries[current_stage - 1] if current_stage > 0 else 0
        stage_len = args.stage_episodes[current_stage]
        stage_progress = (ep - stage_start) / max(1, stage_len)
        epsilon = args.eps_start + min(1.0, stage_progress * (args.eps_decay_episodes / stage_len)) * (args.eps_end - args.eps_start)
        epsilon = max(args.eps_end, epsilon)

        ep_data, ep_return = collect_episode(
            OBELIX, args, args.seed + ep, q_net, device, epsilon, difficulty, wall_obstacles
        )
        ...
        with open(train_log, "a", newline="") as f:
            csv.writer(f).writerow([
                ...
                current_stage + 1, difficulty, int(wall_obstacles),
