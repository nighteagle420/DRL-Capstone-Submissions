...
def main():
    ...
    ap.add_argument("--out", type=str, default="gpuweights.pth")
    ...
    q = FCDuelingNetwork().to(device)
    tgt = FCDuelingNetwork().to(device)
    ...
    for ep in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            if np.random.rand() < eps:
                ...
                with torch.no_grad():
                    qs = q(torch.tensor(s, dtype=torch.float32,device=device).unsqueeze(0)).squeeze(0).cpu().numpy()
                a = int(np.argmax(qs))
            ...
            if len(replay) >= max(args.warmup, args.batch):
                ...
                if steps % args.target_sync == 0:
                    # tgt.load_state_dict(q.state_dict())
                    with torch.no_grad():
                        for target,online in zip(tgt.parameters(),q.parameters()):
                            target_ratio = (1.0 - args.tau) * target
                            online_ratio = args.tau * online

                            mixed_weights = target_ratio + online_ratio
                            target.copy_(mixed_weights)
