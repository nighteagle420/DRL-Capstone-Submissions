...
class Replay:
    ...
    def sample(self, batch: int):
        ...
        obs = np.stack([it.obs for it in items]).astype(np.float32)
        att = np.array([it.attached for it in items], dtype=np.float32)

        return s, a, r, s2, d, obs, att
    def __len__(self):
        return len(self.buf)
...
def evaluate_policy(q, attach_net, OBELIX, args, device, base_seed, episodes=10):
    q.eval()
    attach_net.eval()
    ...
    for i in range(episodes):
        ...
        while not done:
            with torch.no_grad():
                ...
                att = torch.sigmoid(attach_net(obs_t))
                s = torch.cat([obs_t, att], dim=1)
    ...
    for ep in tqdm(range(args.episodes),leave=True):
        ...
        for _ in range(args.max_steps):
            ...
            replay.add(Transition(
                s=s, a=a, r=r, s2=s2, done=done,
                obs=obs, attached=ATTACHED
            ))
            s = s2
            obs = obs2
            steps += 1
            ...
            if len(replay) >= max(args.warmup, args.batch):
                sb, ab, rb, s2b, db, obsb, attb = replay.sample(args.batch)
                ...
                opt.step()
                 ...
                 # AttachNet training
                obsb_t = torch.tensor(obsb, device=device)
                attb_t = torch.tensor(attb, device=device).unsqueeze(1)

                logits = attach_net(obsb_t)
                loss_att = bce(logits, attb_t)

                attach_opt.zero_grad()
                loss_att.backward()
                nn.utils.clip_grad_norm_(attach_net.parameters(), 5.0)
                attach_opt.step()
        ...
        if (ep + 1) % 50 == 0:
            eval_score = evaluate_policy(q, attach_net,OBELIX, args, device, args.seed)
