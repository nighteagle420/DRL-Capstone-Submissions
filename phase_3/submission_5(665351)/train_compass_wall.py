...
def wall_probability(ep, total_eps, wall_start_frac=0.2, wall_full_frac=0.7):
    if ep < total_eps * wall_start_frac:
        return 0.0
    elif ep > total_eps * wall_full_frac:
        return 1.0
    else:
        progress = (ep - total_eps * wall_start_frac) / (total_eps * (wall_full_frac - wall_start_frac))
        return progress
...
def main():
    ...
    ap.add_argument("--wall_start_frac", type=float, default=0.2)
    ap.add_argument("--wall_full_frac", type=float, default=0.7)

    ap.add_argument("--seed", type=int, default=0)
    args = ap.parse_args()

    print(f"UNIFIED DDQN WALL-SCHEDULE | DIFF={args.difficulty}, "
          f"WALL_START={args.wall_start_frac}, WALL_FULL={args.wall_full_frac}")
    ...
    with open(log_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["episode", "return", "epsilon", "replay_size", "steps", "pred_loss", "wall_prob", "use_wall"])
    ...
    for ep in tqdm(range(args.episodes), leave=True):
        wp = wall_probability(ep, args.episodes, args.wall_start_frac, args.wall_full_frac)
        use_wall = bool(np.random.rand() < wp)
        ...
        with open(log_file, "a", newline="") as f:
            ...
            writer.writerow([ep + 1, ep_ret, eps_by_step(steps), len(replay), steps,
                             f"{latest_pred_loss:.4f}", f"{wp:.3f}", int(use_wall)])
        ...
        if (ep + 1) % 50 == 0:
            tqdm.write(f"Episode {ep+1}/{args.episodes} return={ep_ret:.1f} pred_loss={latest_pred_loss:.4f} "
                       f"wall_prob={wp:.2f} use_wall={use_wall}")
