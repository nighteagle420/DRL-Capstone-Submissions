...
def main():
    ...
    for ep in tqdm(range(args.episodes), leave=True):
        ...
        for _ in range(args.max_steps):
            ...
            r_shaped = max(r, -10.0)
            is_blind = obs_raw_next[:17].sum() == 0 and not attached
            if is_blind and a == 2 and obs_raw_next[17] == 0:
                blind_fw_streak += 1
                bonus = max(0.0, 0.6 - 0.1 * blind_fw_streak)
                r_shaped += bonus
            else:
                blind_fw_streak = 0
            replay.add(Transition(s=s, a=a, r=r_shaped, s2=s2, done=bool(done)))
