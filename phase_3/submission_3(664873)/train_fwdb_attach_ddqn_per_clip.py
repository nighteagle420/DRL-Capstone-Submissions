...
def augment_obs(obs_raw, attached_bit):
    return np.concatenate([obs_raw, [float(attached_bit)]])
...
def main():
    ...
    ap.add_argument("--spin_thresh", type=int, default=5)
    ap.add_argument("--spin_penalty", type=float, default=0.1)

    ap.add_argument("--seed", type=int, default=0)
    args = ap.parse_args()
    args.wall_obstacles = True

    print(f"WALL OBSTACLES - {args.wall_obstacles}, DIFFICULTY - {args.difficulty}, SPIN_THRESH - {args.spin_thresh}, SPIN_PENALTY - {args.spin_penalty}")
    ...
    for ep in tqdm(range(args.episodes), leave=True):
        ...
        attached = 0
        obs_aug = augment_obs(obs_raw, attached)
        stacker = FrameStack(N_FRAMES, OBS_AUG)
        s = stacker.reset(obs_aug)

        ep_ret = 0.0
        episode_traj = []
        consec_blind_rotations = 0
        ...
        for _ in range(args.max_steps):
            ...
            blind = not obs_raw_next[:17].any()
            shaped_r = r

            if attached == 0 and blind:
                if a != 2:
                    consec_blind_rotations += 1
                else:
                    consec_blind_rotations = 0

                if consec_blind_rotations > args.spin_thresh:
                    shaped_r -= args.spin_penalty * (consec_blind_rotations - args.spin_thresh)
            else:
                consec_blind_rotations = 0

            obs_aug_next = augment_obs(obs_raw_next, attached)
            s2 = stacker.push(obs_aug_next)

            replay.add(Transition(s=s, a=a, r=max(shaped_r, -10.0), s2=s2, done=bool(done)))
            s = s2
