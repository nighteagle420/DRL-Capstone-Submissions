...
class NStepBuffer:
    ...
    def get(self):
        ...
        _, _, _, s_last, done_last = self.buf[-1]
        n_step_return = 0.0
        for i in range(len(self.buf)):
            _, _, r_i, _, d_i = self.buf[i]
            n_step_return += (self.gamma ** i) * r_i
            if d_i:
                return s0, a0, n_step_return, self.buf[i][3], True
        return s0, a0, n_step_return, s_last, done_last
    ...
    def flush(self):
        ...
        while len(self.buf) > 0:
            ...
            n_step_return = 0.0
            done_flag = False
            last_s2 = self.buf[-1][3]
            for i in range(len(self.buf)):
                _, _, r_i, _, d_i = self.buf[i]
                n_step_return += (self.gamma ** i) * r_i
                if d_i:
                    last_s2 = self.buf[i][3]
                    done_flag = True
                    break
            transitions.append((s0, a0, n_step_return, last_s2, done_flag))
            self.buf.popleft()
        return transitions
...
def main():
    ...
    ap.add_argument("--eps_decay_steps", type=int, default=2000000)

    ap.add_argument("--seed", type=int, default=0)
    args = ap.parse_args()
    args.wall_obstacles = True

    print(f"FIND-DDQN TRAINING | WALL={args.wall_obstacles}, DIFF={args.difficulty}, N_STEP={args.n_step}, EPS_DECAY={args.eps_decay_steps}")
    ...
    for ep in tqdm(range(args.episodes), leave=True):
        ...
        ep_ret = 0.0
        attached = False
        nstep = NStepBuffer(n_step=args.n_step, gamma=args.gamma)
        ...
        for _ in range(args.max_steps):
            ...
            nstep.append(s, a, max(r, -10.0), s2, bool(done))

            if nstep.is_ready():
                ns, na, nr, ns2, nd = nstep.get()
                replay.add(Transition(s=ns, a=na, r=nr, s2=ns2, done=nd))
            ...
            if done:
                for ns, na, nr, ns2, nd in nstep.flush():
                    replay.add(Transition(s=ns, a=na, r=nr, s2=ns2, done=nd))
                break
